...
class pathDisclosure(baseGrepPlugin):
    ...
    def _compile_regex(self):
        '''
        Only ONE regular expression is compiled. Based on my tests, it's better to have
        one regular expression like this:

            (regex_a|regex_b|...|regex_n)
            
        Instead of doing this:
        
            regex_list = [regex_a, regex_b, ... ,regex_n]
            for regex in regex_list:
                re.match( regex , the_string )
                
        The first reads the string only once (if you don't have backtracking).
        
        @return: None, the result is saved in self._path_disclosure_regex
        '''
        self._path_disclosure_regex = '('
        
        for path_disclosure_string in self._get_path_disclosure_strings():
            self._path_disclosure_regex += '('+path_disclosure_string + '.*?)[^A-Za-z0-9\._\-\\/\+~]|'
        
        self._path_disclosure_regex = self._path_disclosure_regex[:-1]
        self._path_disclosure_regex += ')'
        self._path_disclosure_regex = re.compile( self._path_disclosure_regex, re.IGNORECASE )
    ...
    def grep(self, request, response):
        ...
        if response.is_text_or_html():
            ...
            #
            #   Please note that we use response.getText() because we want
            #   to find path disclosure vulnerabilities in the text elements of
            #   the HTML. Trying to find them in attribute values raises
            #   tons of false positives.
            #
            match_list = self._path_disclosure_regex.findall( response.getText()  )
            
            if match_list:
                match_list = list(match_list[0])
                match_list = [ i.strip() for i in match_list if len(i)]
    ...
    def _attr_value(self, path_disclosure_string, response ):
        '''
        IMPORTANT/
            This function isn't used anymore. It's here as a reference on how 
            getDOM() and xpath() can be used.
        /IMPORTANT
        
        This method was created to remove some false positives.
        
        @parameter path_disclosure_string: The string that we think is a path disclosure.
        @parameter response: The HTTP response object.
        ...
        dom = response.getDOM()
        if dom == None:
            # I'm not able to tell if this is an attr value or not,
            # my default is going to be False.
            res = False
        else:
            # Lets find it with xpath:
            #
            #   >>> e = etree.fromstring( '<a><b c="/home/im.png"></b></a>' )
            #   >>> e.xpath("//@*='/home/im.png'")
            #   True
            #   >>> 
            
            #   TODO:
            #   Make sure we don't have an xpath injection in w3af :)
            #
            # path_disclosure_string = 
            xpath_expression = "//@*='" + path_disclosure_string + "'"
            
            try:
                res = dom.xpath( xpath_expression )
            except Exception, e:
                return False
