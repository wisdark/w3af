...
class directoryIndexing(baseGrepPlugin):
    ...
    def __init__(self):
        ...
        # Added performance by compiling all the regular expressions into one.
        regex_list = self._get_indexing_regex()
        regex_list.sort()
        regex = '(' + '|'.join( regex_list ) + ')'
        self._compiled_regex = re.compile(regex, re.IGNORECASE | re.DOTALL)
    ...
    def grep(self, request, response):
        ...
        if response.is_text_or_html():
            ...
            if urlParser.getDomainPath(response.getURL()) in self._already_visited:
                ...
                match_list = self._compiled_regex.findall( html_string )
