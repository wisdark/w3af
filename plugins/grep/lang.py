...
class lang(baseGrepPlugin):
    ...
    def __init__(self):
        ...
        self._compiled_regex_list = []
        self._compile_regex()
        
    def _compile_regex(self):
        '''
        @return: None, the result is saved in self._compiled_regex_list
        '''
        for lang_string in self._prepositions:
            ...
            self._compiled_regex_list.append( (lang_string, re.compile(preposition_regex) ) )
    ...
    def grep(self, request, response):
        ...
        with self._plugin_lock:
            if self._exec and response.is_text_or_html() and not is_404( response ):
                ...
                number_of_matches = {}
                #response_text = response.getText().lower()
                word_index = response.getWordIndex()
                
                for lang_string in self._prepositions:
                    number_of_matches[ lang_string ] = 0
                    
                    for preposition in self._prepositions[lang_string]:
                        number_of_matches[ lang_string ] += word_index.get(preposition, 0)
                
                #for lang_string, preposition_regex in self._compiled_regex_list:
                    
                    # Init the count map
                 #   number_of_matches[ lang_string ] = 0
                 #   matches = preposition_regex.findall( response_text)
