...
class httpResponse:
    ...
    def getText(self):
        '''
        @return: Text only representation of the response document.
        '''
        if self._text:
            return self._text
        else:
    ...
    def getWordIndex(self):
        '''
        Creates and index of words that holds the words as keys and the number
        of repetitions as values. If the index is not created, it creates it.
        
        @return: A dict with {'word': repetitions}
        '''
        if self._word_index:
            ...
            splitted = split_re.split( self.getText() )
            
            for word in splitted:
                if word in self._word_index:
                    self._word_index[ word ] += 1
                else:
                    self._word_index[ word ] = 0
