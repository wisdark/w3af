...
from ..auto_update import W3afSVNClient, Revision, FILE_UPD, FILE_NEW, \
    FILE_DEL, ST_CONFLICT, ST_MODIFIED, ST_UNKNOWN
...
class TestW3afSVNClient(PyMockTestCase):
    ...
    upd_files = [('file1.txt', FILE_NEW), ('file2.py', FILE_UPD),
                 ('file3.jpg', FILE_DEL)]
    ...
    def test_upd(self):
        client = self.client
        rev = Revision(1290, None)
        method(client._svnclient, 'update').expects(LOCAL_PATH).returns([rev])
        override(client, '_filter_files').expects(client.UPD_ACTIONS)
        self.returns(self.upd_files)
        ## Stop recording. Play!
        self.replay()
        self.assertEquals((rev, self.upd_files), client.update())
        ## Verify ##
        self.verify()
    ...
    def test_filter_files(self):
        ...
        expected_res = [(f1, FILE_DEL), (f2, FILE_UPD)]
    ...
    def test_status(self):
        ...
        self.replay()
        expected_res = \
            [('/some/path/foo', ST_MODIFIED), ('/some/path/foo2', ST_CONFLICT),
